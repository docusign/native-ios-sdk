...
/*! boolean value to determine if next button is shown instead of create to transition to next signature capture. Default value is `false`.
*/
@property (nonatomic, assign) BOOL shouldShowNextButton;
/*! boolean value to determine if signature capture is being done for offline signing. Setting offlineMode to `true` would ensure that no API calls are made during signature capture. Only supported methods are `Draw` and `Take Photo`. The default value is `false`.
*/
